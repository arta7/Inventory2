...
const FiscalYears: React.FC = () => {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (text, record, index) => < div className="btn-wrap"
        ...
        <Popconfirm title="آیا مطمئن هستید?" onConfirm={() =>      DeleteFiscalYear(record.Id)}>
            < Button
  ...
  let AddFiscalYear = () => {
    ...
    axios.post(Config.URL +
      Config.Defination.AddFiscalYear, data)
      .then((response) => {
  ...
  let DeleteFiscalYear = (_id) => {
    ...
    axios.post(Config.URL +
      Config.Defination.DeleteFiscalYear, data)
      .then((response) => {
        console.log('response data : ', response.data.data)
        setCounter(Counter+1)
      })
          ...
          <BaseForm layout="vertical" onFinish={handleSubmit} form={form}>
            ...
            <div style={{ flexDirection: 'row', justifyContent: 'space-between', display: 'flex' }}>
              ...
              <Auth.SubmitButton type="primary" loading={isLoading} style={{ marginRight: 10 }} onClick={() => {
                ...
                if (Titles.toString().trim().length > 0 && Code.toString().trim().length>0)
                AddFiscalYear()
                else
                {
